...
On the **Component** tab you'll see four default component-level tokens — `button-disabled`, `input-disabled`, `modal-backdrop`, and `placeholder-text` — each pointing at a semantic opacity token. This final layer makes it trivial to adjust the disabled opacity for all form elements by changing a single semantic token. Pro users can customize these mappings.
